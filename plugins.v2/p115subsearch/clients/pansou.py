...
class PanSouClient:
    ...
    def check_links(self, items: List[Dict[str, str]]) -> List[Dict[str, Any]]:
        """
        批量检测网盘分享链接有效性（渠道无关，当前仅支持 115 链接）
        ...
        try:
            ...
            self._api_call_count += 1
            response = requests.post(check_url, json=payload, headers=headers, timeout=8, proxies=self._proxies)
            ...
            if response.status_code != 200:
                logger.warning(f"PanSou 链接检测请求失败: HTTP {response.status_code}，降级为原有校验流程")
                return []

            resp_data = response.json()
            results = resp_data.get("results", [])
            if not isinstance(results, list):
                logger.warning("PanSou 链接检测响应格式异常，降级为原有校验流程")
                return []
